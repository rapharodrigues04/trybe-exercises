...
function sendMarsTemperature(sendMessage, erroMessage) {
    const currentTemp = getMarsTemperature();
    const messageSendSucess = Math.random() <= 0.6;
    setTimeout(() => {
        if(messageSendSucess) sendMessage(currentTemp);
        else {
            erroMessage('Robot is busy');
        }
    })
}
